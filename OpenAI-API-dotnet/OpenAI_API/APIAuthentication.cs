...
namespace OpenAI_API
{
	...
	public class APIAuthentication
	{
        ...
        public APIAuthentication(IConfigurationSection configuration)
        {
            this.ApiKey = configuration["ApiKey"];
			this.OpenAIOrganization= configuration["OrganizationID"];

			this.BaseUrlFormat = configuration["BaseUrlFormat"] ?? "https://api.openai.com/{0}/{1}";


 
			bool.TryParse(configuration["EnableProxy"], out var _enableProxy);
			this.EnableProxy = _enableProxy;

			if(this.EnableProxy)
			{
                bool.TryParse(configuration["BypassProxyOnLocal"], out var _BypassProxyOnLocal);
                this.BypassProxyOnLocal = _BypassProxyOnLocal;

                bool.TryParse(configuration["UseSystemProxy"], out var _UseSystemProxy);
                this.UseSystemProxy = _UseSystemProxy;
				if (!this.UseSystemProxy)
				{
					this.ProxyUrl = configuration["ProxyUrl"]??string.Empty;
                    this.ProxyUsername = configuration["ProxyUsername"] ?? string.Empty;
                    this.ProxyPassword = configuration["ProxyPassword"] ?? string.Empty;                
                }
            }
