	...
	public abstract class EndpointBase
	{
		...
		protected HttpClient GetClient()
		{
			...
			*/
			...
			HttpClient client;
            #region 初始化 API地址 及 HttpClient Eddy@20230327

            if (!string.IsNullOrEmpty(_Api.Auth.BaseUrlFormat))
            {
                this._Api.ApiUrlFormat = _Api.Auth.BaseUrlFormat;
            }

			if (_Api.Auth.EnableProxy)
			{
				var proxy = new WebProxy
				{
					Address = new Uri(_Api.Auth.ProxyUrl),
					BypassProxyOnLocal = _Api.Auth.BypassProxyOnLocal,
					UseDefaultCredentials = true

				};
				if (!_Api.Auth.UseSystemProxy && !string.IsNullOrEmpty(_Api.Auth.ProxyUsername) && !string.IsNullOrEmpty(_Api.Auth.ProxyPassword))
				{
					proxy.UseDefaultCredentials = false;
					proxy.Credentials = new NetworkCredential(_Api.Auth.ProxyUsername, _Api.Auth.ProxyPassword);
				}
				...
				{
					Proxy = proxy,
					UseProxy = _Api.Auth.EnableProxy,
				...
				handler.SslProtocols = System.Security.Authentication.SslProtocols.Tls12 | System.Security.Authentication.SslProtocols.Tls | System.Security.Authentication.SslProtocols.Tls11 |
					System.Security.Authentication.SslProtocols.Ssl3 | System.Security.Authentication.SslProtocols.Ssl2;
